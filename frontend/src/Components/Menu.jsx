...
function Menu({category,setcategory}){
  ...
  return (
    <div className='flex flex-col ml-8 m-6 pb-16 border-b-2 border-gray-200'>
        ...
        <Slider {...settings} >
            {menu_list.map((items)=>(
                <div key={items.index} className='flex flex-col justify-center mt-2 ml-1 '>
                    <button onClick={()=>{setcategory(items.menu_name)}}>{category===items.menu_name?<img src={items.menu_image} className='rounded-full border-4 border-red-700 ' alt="" />:<img className='hover:scale-110' src={items.menu_image}/>}
                    </button>
                    <h1 className='sm:text-start sm:pl-10 text-center text-sm'>{items.menu_name}</h1>
                    </div>
            ))}
        </Slider>
    </div>
